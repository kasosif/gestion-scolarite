...
<div class="container" style="height: 100%; position:relative; margin-top: 25px;">
    <div>
        ...
        <p>Inscrit(e) dans la spécialité <strong>{{ $etudiant->classe->niveau->specialite->nom }} </strong> sous le numéro <strong>{{ $etudiant->classe->promotion }}0{{ $etudiant->classe->niveau->specialite->code }}{{ $etudiant->id  }}</strong>, est inscrit en "{{ $etudiant->classe->niveau->nom  }}  {{ $etudiant->classe->abbreviation  }}" et ce au titre de l’année scolaire {{ $etudiant->classe->niveau->specialite->annee->nom }}.
        </p>
        <p>
